...
# Modelo de base de datos para el formulario
...
# Endpoint para recibir el formulario y guardar en la base de datos
@app.post("/send-form/")
def receive_form(name: str = Form(...), phone: str = Form(...), email: str = Form(...), message: str = Form(...)):
    db = SessionLocal()
    form_entry = FormEntry(name=name, phone=phone, email=email, message=message)
    db.add(form_entry)
    db.commit()
